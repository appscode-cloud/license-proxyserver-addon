...
PRODUCT_NAME       := license-proxyserver-addon
ENFORCE_LICENSE    ?=

GO_PKG   := go.bytebuilder.dev
REPO     := $(notdir $(shell pwd))
BIN      := license-proxyserver-addon
COMPRESS ?= no
...
install:
	...
	helm upgrade -i license-proxyserver-addon-manager charts/license-proxyserver-addon-manager --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall license-proxyserver-addon-manager --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/license-proxyserver-addon run \
		--v=3 \
		--secure-port=8443 \
